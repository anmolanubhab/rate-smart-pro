...
-- Same defect as receive_sales_payment (just fixed): supplier_payment_
-- allocations rows (bumping purchase_invoices.paid_amount via their own
-- trigger) were inserted unconditionally, while the voucher-posting block
-- was gated `IF v_supplier_ledger IS NOT NULL AND v_pay_ledger IS NOT NULL`
-- -- a supplier payment whose ledgers couldn't be resolved would still mark
-- the invoice paid/partially-paid with zero GL effect. Fix: resolve and
-- validate both ledgers up front; RAISE (rolling back everything already
-- inserted in this call, same one-function-one-transaction guarantee) if
-- either is missing.
CREATE OR REPLACE FUNCTION public.pay_supplier_bills(_business_id uuid, _supplier_id uuid, _amount numeric, _mode text, _payment_date date, _reference_note text, _allocations jsonb)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
