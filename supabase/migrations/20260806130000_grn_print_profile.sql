-- Universal Document Output Center, Phase 4: GRN never had a print_profiles
-- document_type before. GRN is a pure quantity-reconciliation document (no
-- pricing at all — ordered/received/damaged/accepted/pending/short/excess
-- qty only), so its default profile has show_rate/show_gst_summary/
-- show_amount/show_discount all false, matching what a GRN actually shows
-- today (this was also the phase where PurchaseGRN.tsx's print was found to
-- be broken — no line items printed at all, since it never had a
-- DocumentGridPrintTable counterpart to its editable grid; this migration
-- is half of the fix, the other half is the getUdm() bridge).
...
ALTER TABLE public.print_profiles ADD CONSTRAINT print_profiles_document_type_check
  CHECK (document_type IN (
    ...
    'journal_voucher', 'contra_voucher', 'sales_return', 'grn'
  ));
