-- Stock Take QA: posting a sheet where no counted-variance line has a
-- cost_price/purchase_price set (so there's nothing to value) always failed
-- with "Cannot hard-delete posted voucher ... it was generated from a
-- stock_take." post_stock_take() inserted its scratch voucher as status =
-- 'posted' up front, then tried to DELETE it once it found there was no
-- cost-valued line to keep — but trg_prevent_posted_voucher_delete
-- (added later, for legitimate reasons) blocks hard-deleting any voucher
-- that's already 'posted' with a reference_type set. Fix: insert the
-- voucher as 'draft' and only flip it to 'posted' once we know we're
-- keeping it; the empty-value path can then delete it as a draft, which
-- the trigger allows.
