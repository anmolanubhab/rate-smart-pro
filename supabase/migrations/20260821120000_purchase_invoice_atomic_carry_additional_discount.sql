-- Follow-up to create_purchase_invoice_atomic (20260819095000) and the
-- Purchase Pricing Engine (20260821110000/20260821110100).
--
-- Bug: linking a PO or GRN into a Purchase Invoice silently dropped the
-- PO's negotiated discount_percent/additional_discount_percent entirely
-- (fetchPendingPOItemsForInvoice/fetchGrnItemsForInvoice never even
-- selected those columns) — a linked invoice re-priced at full rate with
-- zero discount. That's fixed client-side in src/lib/purchaseInvoices.ts.
--
-- This migration closes the remaining gap: even with the fix above,
-- create_purchase_invoice_atomic's INSERT into purchase_invoice_items never
-- wrote additional_discount_percent (or the pricing-engine snapshot
-- columns) — the financial totals were already correct (taxable_amount/
-- tax_amount/total_amount are trusted from the client, unaffected), but the
-- persisted row's own additional_discount_percent stayed 0, breaking the
-- "transaction snapshot never drifts" guarantee for a linked invoice.
