...
import { Calculator as CalcIcon, TrendingUp, Activity, ArrowRight, Percent, Wallet } from "lucide-react";
...
// The original RD-discount calculator analytics — the product this app
// started as, before the ERP layers above it existed. Kept as its own
// lazy, self-fetching panel (moved out of Dashboard.tsx) so it only loads
// and only queries `calculations` when the Reports tab is actually opened.
...
export default function LegacyCalculatorPanel() {
  ...
  useEffect(() => {
    if (!user) return;
    let q = supabase.from("calculations").select("*").order("created_at", { ascending: false });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display text-xl font-bold">RD Calculator Analytics</h2>
        <p className="text-sm text-muted-foreground">Bill-discount calculations — the original RD Pro tool.</p>
      </div>
