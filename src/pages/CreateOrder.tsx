    ...
    <div className="sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm">
      <div className="px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={onAddRow} className="gap-2"><Plus className="w-4 h-4" />Add Row</Button>
          <Button variant="outline" size="sm" onClick={onUpload} className="gap-2"><Upload className="w-4 h-4" />Upload</Button>
          <Button variant="outline" size="sm" onClick={onPrint} className="gap-2"><Printer className="w-4 h-4" />Print</Button>
          <Button variant="outline" size="sm" onClick={onDownloadPDF} className="gap-2"><Download className="w-4 h-4" />PDF</Button>
          <Button variant="outline" size="sm" onClick={onSaveDraft} className="gap-2"><Save className="w-4 h-4" />Save Draft</Button>
          <Button size="sm" onClick={onConfirm} className="gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white shadow-sm transition-all"><CheckCircle className="w-4 h-4" />Confirm Invoice</Button>
        </div>

        {showShortcuts && (
          <div className="absolute top-full right-0 mt-2 w-80 bg-white rounded-xl shadow-sm border border-gray-200 p-4 z-50">
            <div className="text-sm font-semibold mb-3">Keyboard Shortcuts</div>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span>Save Draft</span><span>Ctrl + S</span></div>
              <div className="flex justify-between"><span>Print Invoice</span><span>Ctrl + P</span></div>
              <div className="flex justify-between"><span>Confirm Invoice</span><span>Ctrl + Enter</span></div>
              <div className="flex justify-between"><span>Add New Row</span><span>Ctrl + Shift + A</span></div>
            </div>
          </div>
        )}
      </div>
          ...
          <div>
            <div className="relative">
              ...
              {isOpen && filteredParties.length > 0 && (
                <div className="absolute top-full left-0 mt-2 w-96 bg-white rounded-xl shadow-sm border border-gray-200 z-50 max-h-96 overflow-auto">
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-gray-50 border-b-2 border-gray-200 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
            ...
            <th className="px-3 py-3 min-w-[140px]">Part Number (F2)</th>
              ...
              <tr key={idx} className={cn("border-b border-gray-100 transition-colors", focusedCell?.row === idx && "bg-blue-50/30", isDup && "bg-amber-50", isLowStock && "bg-orange-50/30")}>
                ...
                <td className="px-1 py-2 relative">
                  <Input ref={(el: any) => { inputRefs.current[`${idx}-part`] = el; }} value={item.part_number} 
                    onChange={(e: any) => onUpdateRow(idx, { part_number: e.target.value.toUpperCase() })} onFocus={() => setFocusedCell({ row: idx, col: 'part' })} onKeyDown={(e: any) => handleKeyDown(e, idx, 'part')} placeholder="Type or F2..." className="h-9 font-mono" />
        ...
        {isExpanded && (
          <div className="p-6 space-y-4">
            ...
            <div className="mt-4 space-y-2">
              <button className="w-full py-3 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-xl font-semibold shadow-sm"><Banknote className="w-4 h-4 inline mr-2" />Collect Payment</button>
            </div>
          </div>
        )}
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <ProductGrid items={items} dupSet={dupSet} onUpdateRow={updateRow} onDeleteRow={delRow} onAddRow={() => setItems((r) => [...r, blankRow()])} defaultDiscount={defaultDiscount} />
              </div>
