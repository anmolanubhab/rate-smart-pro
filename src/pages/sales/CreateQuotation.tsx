...
import { Save, FileCheck2, Printer, FileDown, Plus, Trash2, ArrowRightCircle, History } from "lucide-react";
...
import {
  nextQuotationNumber, saveQuotation, fetchQuotationById, fetchQuotationItems, fetchQuotationRevisions,
  convertQuotationToOrder, type Quotation, type QuotationStatus,
} from "@/lib/quotations";
...
const CreateQuotation = () => {
  ...
  const [rootQuotationId, setRootQuotationId] = useState<string | null>(null);
    ...
    if (!editId) {
      ...
      (async () => {
        try {
          ...
          setRevisionNumber(q.revision_number);
          setRootQuotationId(q.root_quotation_id);
          setDraftId(q.id);
  ...
  const handleSave = async (status: "draft" | "sent" = "draft") => {
    ...
    try {
      ...
      // saveQuotation may have spun this save off into a new revision row
      // (editing a quotation the customer already saw) — pick up whatever
      // id/status/revision it actually landed on rather than assuming it's
      // the same row we started with.
      setDraftId(saved.id);
      draftIdRef.current = saved.id;
      if (saved.quotation_number) setQuotationNumber(saved.quotation_number);
      setEditStatus(saved.status);
      setRevisionNumber(saved.revision_number);
      setRootQuotationId(saved.root_quotation_id);

      if (status === "sent") {
        toast.success("Quotation sent");
        navigate(`/sales/quotations?highlight=${saved.id}`);
      } else {
        toast.success(status === "draft" ? "Draft saved" : "Saved", { duration: 1500 });
      }
    } catch (e: any) {
      toast.error(e.message);
    } finally {
  ...
  const handleViewHistory = async () => {
    if (!rootQuotationId) return;
    setHistoryOpen(true);
    setHistoryLoading(true);
    try {
      setHistoryRows(await fetchQuotationRevisions(rootQuotationId));
    } catch (e: any) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      // Once the quotation has left draft, a save spins off a new revision —
      // the idle 30s autosave must not do that on its own with no real edit.
      if (editMode && editStatus !== "draft") return;
      if (Date.now() - lastSavedAt.current < 25000) return;
      lastSavedAt.current = Date.now();
      handleSave("draft");
    }, 30000);
    return () => clearInterval(id);
  }, [items, partyId, user, isConverted, editMode, editStatus]);
  ...
  const toolbarActions: DocumentToolbarAction[] = [
    ...
    { key: "history", label: "History", icon: History, onClick: handleViewHistory, hidden: revisionNumber === 0 },
    ...
    <DocumentRoot type="quotation" printTitle="QUOTATION">
      <DocumentToolbar
        statusSlot={
          <>
            ...
            {revisionNumber > 0 && <Badge variant="outline" className="text-[10px]">Rev {revisionNumber}</Badge>}
      ...
      <Dialog open={historyOpen} onOpenChange={setHistoryOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Revision History — {quotationNumber}</DialogTitle>
          </DialogHeader>
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {historyLoading ? (
                <TableRow><TableCell colSpan={4} className="text-center py-6 text-sm text-muted-foreground">Loading…</TableCell></TableRow>
              ) : historyRows.map((r) => (
                <TableRow key={r.id}>
                  <TableCell>
                  ...
                  <TableCell className="text-right font-semibold">₹{fmt(r.grand_total)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
