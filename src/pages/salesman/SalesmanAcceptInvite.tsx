...
import { ShieldCheck, Eye, EyeOff, XCircle, CheckCircle2, Briefcase } from "lucide-react";
...
import { getSalesmanInvitationByToken, acceptSalesmanInvitation } from "@/lib/salesmanPortalAccess";
...
export default function SalesmanAcceptInvite() {
  useEffect(() => { document.title = "Accept Invitation — Salesman Portal"; }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await getSalesmanInvitationByToken(token);
  ...
  const finishAccept = async () => {
    setSubmitting(true);
    try {
      await acceptSalesmanInvitation(token);
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-[460px]">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Salesman Portal</h1>
          <p className="text-sm text-slate-500 mt-1">Portal Invitation</p>
        </div>
    ...
    return (
      <Shell>
        <div className="text-center py-4">
          ...
          <p className="text-sm text-slate-500 mt-1">This link is invalid. Please ask your admin to send a new one.</p>
          <Button className="mt-5" variant="outline" onClick={() => navigate("/salesman/login")}>Go to sign in</Button>
        </div>
      </Shell>
    );
  }
    ...
    return (
      <Shell>
        <div className="text-center py-4">
          ...
          <p className="text-sm text-slate-500 mt-1">Ask your admin at <span className="font-medium">{invite.business_name}</span> to resend your invitation.</p>
        </div>
      </Shell>
    );
  }
    ...
    return (
      <Shell>
        <div className="text-center py-4">
          ...
          <Button className="mt-5" onClick={() => navigate("/salesman/login")}>Go to sign in</Button>
        </div>
      </Shell>
    );
  }
    ...
    <Shell>
      <div className="flex items-center gap-2 mb-4 p-3 rounded-xl bg-blue-50 border border-blue-100">
        ...
        <div className="text-sm">
          ...
          <span className="text-slate-500"> invited you to the Salesman Portal as </span>
          <span className="font-medium text-slate-900">{invite.salesman_name}</span>
        </div>
      </div>
      ...
      {isMatchingSession ? (
        <div className="space-y-4">
          <p className="text-sm text-slate-600">
            You're signed in as <span className="font-medium">{user.email}</span>. Activate your Salesman Portal access now?
          </p>
          <Button className="w-full bg-blue-600 hover:bg-blue-700" onClick={handleJoinExisting} disabled={submitting}>
        ...
        <div className="space-y-4">
          ...
          <Button className="w-full bg-blue-600 hover:bg-blue-700" onClick={handleCreateAccount} disabled={submitting}>
