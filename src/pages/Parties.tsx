...
const Parties = () => {
  ...
  const openEdit = (p: Party) => {
    ...
    setForm({
      ...
      preferred_supplier: p.preferred_supplier ?? false,
      preferred_customer: p.preferred_customer ?? false,
      firm_name: p.firm_name || "",
      contact_person: p.contact_person || "",
      alt_phone: p.alt_phone || "",
      email: p.email || "",
      website: p.website || "",
      business_type: p.business_type || "Retailer",
      industry_segment: p.industry_segment || "Automobile Parts",
      pan: p.pan || "",
      msme: p.msme || "",
      ...
      ledger_name: p.ledger_name || p.name,
      opening_balance: String(p.opening_balance ?? 0),
      balance_type: p.balance_type || "CR",
      credit_enabled: p.credit_enabled ?? false,
      credit_days: String(p.credit_days ?? 30),
      interest_pct: String(p.interest_pct ?? 0),
      last_payment_date: p.last_payment_date || "",
      last_invoice_date: p.last_invoice_date || "",
      rate_category: p.rate_category || "Retail Price",
      special_discount: String(p.special_discount ?? 0),
      pricing_notes: p.pricing_notes || "",
      dealer_network: p.dealer_network ?? false,
      online_ordering: p.online_ordering ?? false,
      allow_credit_orders: p.allow_credit_orders ?? false,
      auto_approve: p.auto_approve ?? false,
      network_visibility: p.network_visibility ?? false,
    });
    setActiveTab("general");
    setOpen(true);
  };
