// src/pages/accounts/DebitNote.tsx
...
//   Material Return: always from a Purchase Invoice, reuses Purchase Returns verbatim.
...
export default function DebitNote() {
  useEffect(() => { document.title = "Debit Note — RD Pro"; }, []);

  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        <p className="text-sm text-muted-foreground">Accounts · Notes</p>
        <h1 className="text-2xl font-bold mt-1">Debit Note</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Use <strong>Material Return</strong> when goods are physically going back to a supplier.
      ...
      <Tabs defaultValue="financial_adjustment">
        <TabsList>
          ...
          <TabsTrigger value="material_return">Material Return (Purchase Return based)</TabsTrigger>
        </TabsList>
        <TabsContent value="financial_adjustment" className="pt-4">
          <FinancialAdjustmentListPanel type="Debit Note" hideHeader />
        </TabsContent>
        <TabsContent value="material_return" className="pt-4">
          <ReturnsListPanel kind="purchase" hideHeader />
        </TabsContent>
      </Tabs>
    </div>
  );
